...
def _handle_tool(name, args, request_id):
    try:
        ...
        if name == "sandglass_full_sanity":
            if not _L3:
                return _rpc_error(request_id, -1, "Layer 3 not available")
            return full_sanity()

        if name == "sandglass_stage_brief":
            if not _L3:
                return _rpc_error(request_id, -1, "Layer 3 not available")
            return {"brief": stage_brief()}
...
_TOOLS = [
    ...
    {"name": "sandglass_full_sanity", "description": "Full system health check — L0/L1/L2/L3", "inputSchema": {"type": "object", "properties": {}}},
    {"name": "sandglass_stage_brief", "description": "Stage brief — offset, scenes, decisions summary", "inputSchema": {"type": "object", "properties": {}}},
]
